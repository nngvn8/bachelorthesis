...
Already for only three philosophers the \mdpN has 956 states. When looking at the graphical representation from \pmcvis \redcomment{Figure} it appears to be little helpful for understanding and exploring the graph due to its sheer size. Although \redcomment{Graph} large, this \mdpN is still rather small. With already five philosophers the MDP has about 100 000 states and with 10 philosophers the resulting MDP has more than 8 billion states. The \viewN \viewparamvalident can help to only show the behavior of some Philosophers and hiding the behavior of the remaining ones. 
...
The state \critstate should not be reachable, since it represents the situation of the two systems occupying the unshareable medium at the same time. We will check if this state in fact is not reachable by using the $\viewparamcnf(c(\state))$ where $c(\state) := (x=2) \land (y=2)$. 
...
Firstly we will use the view \viewdistance from that state on. Because in the current version of the project expansion of grouped states to the ones they contain on a visual level, has not been implemented yet, we will use a custom \viewN that emulates this feature.
...
\begin{definition}
	...
	\]
	and $\imggrp = \states \cup \remstates$.
\end{definition}
\sloppy
We will use partial application with $ \viewparamcnf(c(\state)) \compselect \viewident$ where $\compselectset = \{(\gfctdistance,1)\}$ to expand that state. The MDP-Graph then looks as in \redcomment{Figure}. It is to see that the state only contains a single state namely with the id seven. In the current version of implementation it is not possible, to obtain the parameter values. With the database file we obtain that this is the state where $x=2$ and $y=1$. Because $x=2$ represents the system 1 one sending and system 2 waiting, we see that is possible for the second  system to begin to send on the medium although it is already occupied by the first system!
...
\begin{tikzpicture}
	\begin{loglogaxis}
		...
		\addplot table [x=StateCount, y=0AvgG, col sep=comma] {/home/martin/Desktop/InActIdentView.csv};
		\addplot table [x=StateCount, y=0AvgG, col sep=comma] {/home/martin/Desktop/OutActIdentView.csv};
		\addplot table [x=StateCount, y=0AvgG, col sep=comma] {/home/martin/Desktop/InActView.csv};
		\addplot table [x=StateCount, y=0AvgG, col sep=comma] {/home/martin/Desktop/OutActView.csv};
		\addplot table [x=StateCount, y=0AvgG, col sep=comma] {/home/martin/Desktop/OutActSetSizeView.csv};
		\addplot table [x=StateCount, y=0AvgG, col sep=comma] {/home/martin/Desktop/VariablesView.csv};
		\addplot table [x=StateCount, y=0AvgG, col sep=comma] {/home/martin/Desktop/VariablesViewDnf.csv};
		\addplot table [x=StateCount, y=0AvgG, col sep=comma] {/home/martin/Desktop/VariablesViewCnf.csv};
		\addplot table [x=StateCount, y=0AvgG, col sep=comma] {/home/martin/Desktop/ReachabilityView.csv};
		\addplot table [x=StateCount, y=0AvgG, col sep=comma] {/home/martin/Desktop/DistanceView.csv};
		\addplot table [x=StateCount, y=0AvgG, col sep=comma] {/home/martin/Desktop/SccView.csv};
		\addplot table [x=StateCount, y=0AvgG, col sep=comma] {/home/martin/Desktop/SccbView.csv};
		\addplot table [x=StateCount, y=0AvgG, col sep=comma] {/home/martin/Desktop/APView.csv};
		\addplot table [x=StateCount, y=0AvgG, col sep=comma] {/home/martin/Desktop/InitView.csv};
		\addplot table [x=StateCount, y=0AvgG, col sep=comma] {/home/martin/Desktop/PropertyView.csv};
