...
In this chapter we want to show how \viewsN can be used in three different usecases. In Section 6.4 we will consider performance aspects.
In the usecases we will see screenshots made in the current version of \pmcvis. (Grouped) states are represented by gray rectangles. The blue labels on them are the mapped values from \grpfctN. These are not always exactly the same as in the formalization. Especially note that $\texttt{\_\_BLANK\_\_} = \remelem$. When parallel composition is used in any form, the values of \grpfctsN in the label are separated with \texttt{|} and are in the same order as they were composed with the \pll operator. For instance, if we formally had $\view[\viewppty_1]\pll\view[\viewppty_2] = \view[\viewppty_1\pll\viewppty_2]$ and for some grouped states \smstates it were true for all of them $\view[\viewppty_1\pll\viewppty_2](\smstate) = (a, b)$,  where $\smstate \in \smstates$, then in the screenshot of \pmcvis the grouped state of the view would receives the label {\color{RoyalBlue}\texttt{a|b}}.
...
One purpose of \viewsN is to simplify \mdpsN to make them easier to understand. Due to the state explosion problem, even fairly simple systems can become very hard to oversee. As an example consider the concurrency problem Dining Philosophers, where n philosophers have to share n-1 forks and each of them needs two to eat. They sit at a round table with forks between them. Each of them has access only to the fork to his left and to his right, if it is not already occupied. When representing the problem with an \mdpN the choice of which fork the philosopher tries to pick is made at random with an uniform distribution. The corresponding \prism file is shown in the appendix (\ref{lst:0601diningphilosophersfull}).

Already for three philosophers the \mdpN has 956 states. Looking at the graphical representation of \pmcvis in Figure \ref{fig:0601Pure}, it seems to be of little help in understanding and exploring the graph due to its sheer size. Although the graph is large, this \mdpN is still rather small. With already five philosophers, the MDP has about 100,000 states, and with 10 philosophers, the resulting MDP has more than 8 billion states. The \viewN \viewparamvalident can help to show only the behavior of some philosophers and hide the behavior of the remaining ones. 
...
The \viewN $\viewparamvalident(\phil{1})$ groups all states that have the same value of (\phil{1}), ignoring the values of the remaining variables. That is, the values of \phil{2} and \phil{3} are hidden, resulting in only the module of philosopher \phil{1} being shown (figure \ref{fig:0601var}). This can be very helpful if only one specific module is of interest, or if the modules have the same structure, as is the case here with Dining Philosophers. 
...
It is also possible to use the view \viewparamvalident to see the interleaved behavior of two or more modules. To see the interleaved behavior of \phil{1} and \phil{2}, we use the parallel composition $\viewparamvalident(\phil{1}) \pll \viewparamvalident(\phil{2})$. This results in the states $\state_1, \state_2$ being grouped if $(\gfctparamvalident (\state_1 \cond \phil{1}), \gfctparamvalident(\state_1 \cond \phil{2})) = (\gfctparamvalident(\state_2 \cond \phil{1}), \gfctparamvalident(\state_2 \cond \phil{2}))$. Hence, only the value of \phil{3} is hidden which results exactly in the desired interleaved model (Figure \ref{fig:0601var+var}).
...
In general, the \viewparamdnf and \viewparamcnf views are very powerful because they allow you to perform any possible operation on variables, since the state space is finite, for \mdpsN checked in practice.
...
In this section we want to show how views can be used for debugging. In this particular case, we assume that we have observed some unwanted behavior or model checking results. We know that some states - that is, some variable assignments - are not allowed. We will check if these states are indeed inaccessible.

We will consider a small \mdpN for which we were given the \prism file. It represents two systems that intend to send information over an unshareable medium. With a probability of $0.8$ a system can establish a connection and with probability of $0.2$ establishing a connection will fail. Once a connection is established, access to the medium is granted only if it is not occupied by the other system. If the medium is not occupied, the system starts sending, otherwise it waits until the medium is free. Transmission termination is modeled with probabilities. There is a 50 percent chance that the transmission will be aborted, and a 50% chance that the transmission will continue. The number 0 indicates that a system is trying to establish a connection, the number 1 indicates that a system has established the connection, the number 2 indicates that the system is sending.

The \critstate state should be unreachable, since it represents the situation where both systems occupy the unshareable medium at the same time. We will verify that this state is indeed unreachable by using the $\view[\pptyparamcnf](c(\state))$ where $c(\state) := (x=2) \land (y=2)$ (Figure \ref{fig:0602Var}). Note that we do not use \viewN \emph{not} in the disregarding form.
...
We observe that this state is reachable because it is shown, and only reachable states are shown in \pmcvis. The question is how and why. To get this information, we should investigate which state has reached this critical state \critstate. One way to do this is to look at the database that stores states and transitions (table \ref{tab:0602database}). For this very simple \mdpN, this seems like a rather tedious approach.
