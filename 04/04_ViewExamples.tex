...
\begin{definition}
		Let $\chgph = \chgphtuple$ be \achgphN, $\smstates \subseteq \states$, $\propfct : \states \to \realnums$ a property function and $Z = \{Z_1, \dots, Z_n\}$ a partition on $\propfct[\states]$. \redcomment{NAME MISSING}
		\[
		\state \mapsto Z_i \quad\quad\quad \text{where } \propfct(\state) \in Z_i \in Z \;\; (i \in \{1, \dots, n\}) 
		\]
\end{definition}
\redcomment{Ideas that came, when writing this part now: Disjunctive composition! Has}

\redcomment{other propperties of mdp, assign to states, usable for views, arbitrary model checker (pctl for example), generates function states to something, function use to define view}
\redcomment{rounding}
