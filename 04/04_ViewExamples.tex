...
\redcomment{Variants of ALGORITHM which allow directionless or only reverse traversal of the \mdpN are provided in the appendix. The respective views would utilize their returned set in the exact same way. That is, \fctdistdefault is replaced with the set of the respective algorithm and nothing else changes.}

\begin{figure}[h]
	\begin{minipage}{.5\textwidth}
		\hspace{15mm}		
		\input{./04/images/distance/distRevAfter}
	\end{minipage}%
	\begin{minipage}{.5\textwidth}
		\hspace{10mm}		
		\input{./04/images/distance/distBiAfter}
	\end{minipage}	
...
In the implementation strongly connected components are determined using the algorithm of Gabow, afterwards filtering those \sccsN that have only transitions to states within the \sccN. Equality, equivalence classes and the new state set behave and are constructed analogously to the \viewN \viewscc.
