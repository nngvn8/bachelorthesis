...
In this chapter we will introduce and discuss some \viewN examples created by the author. Their purpose is to understand the idea and concept of a \viewN and get to know some \viewsN that might be useful in real world applications. \\
...
In this section we will introduce some \viewsN the are purely based on the characteristics of an \mdpN. This includes \viewsN utilizing elements of the tuple from the definition of \chgphsN ("\chgph components"), but also information that is attached to it, such as variables, reward or cost structure and model checking results.
\subsubsection{Atomic Propositions}
One of the least involved approaches to create a \viewN is to base it on the atomic propositions, that are assigned to each state by the labeling function. Atomic Propositions are of relevance because in they are the base tools to identify states with certain notable properties. The notion is to group states that were assigned the same set of atomic propositions.
...
The grouping function is exactly the labeling function i.e. for all $\state \in \states$ it is $\gfctatomicprops(\state) := \labelingfct(\state)$. So it is $\gfctatomicprops(\state_1) = \gfctatomicprops(\state_2)$ \iffN $\labelingfct(\state_1) = \labelingfct(\state_2)$. According to Definition \ref{def:eqrelview} for $\smstate \in \states$ it is $\eqclassv[\smstate] = \{\state \in \states \mid \labelingfct(\state) = \labelingfct(\smstate)\}$.
...
Although this \viewN might seem rather simple, because essentially it only performs $\gfctatomicprops := \labelingfct$ it is the most powerful one. This is because every \viewN presented in the following is reducible to this one. That is because a \grpfctN assigns a value to each state. This value could be interpreted as atomic proposition. The reduction can be realized by replacing the labeling function with the grouping function of the resepective \viewN. That is $\labelingfct := \grpfct$ and $\atomicprops := {\grpfct}[\states]$ for some \grpfctN \grpfct. While this works it alters the underlying \chgphN.

\subsubsection{Initial States}
An a little more involved idea than directly using a given function is to utilize the support of initial distribution $\{\state \in \states \mid \initdistrib(\state) > 0\}$. We can group states that have a probability greater zero, that they are started from. In practice this might be useful to quickly find all initial states.
...
\noindent According to Definition \ref{def:eqrelview} the equivalence classes of \eqrelview on \states are 
\begin{alignat*}{3}
...
Another crucial component of \achgphN is its set of actions \actions. Actions are used for interprocesscommunication und synchronization. In this subsection we will provide and discuss some \viewsN utilizing action outgoing from a state. 
...
\noindent In a similar fashion we define \viewN that groups states where at most a certain number of times a given action is outgoing. 
...
The predicate \predmaxoutact ensures that there are at most \numoutact actions \action outgoing, by requiring that if there exist more than \numoutact transitions outgoing with action \action, at least two of them are the same. It does so by enforcing that the states where the transitions end are in fact the same. The reasoning about the equality of the \grpfctN values, the obtained equivalence classes and the resulting set of states $\states'$ of the \viewN is analogous to \viewminoutaction.
...
\formoutact is true \iffN the number of outgoing actions is in the span. For $\state_1, \state_2 \in \states$ it is $\gfctspanoutaction(\state_1) = \gfctspanoutaction(\state_2)$ \iffN $\formoutact[\state_1] \land \formoutact[\state_2]$ or $\state_1 = \state_2$. According to Definition \ref{def:eqrelview} the equivalence classes of \eqrelview on \states are 
\begin{alignat*}{3}
...
The \grpfctN assigns to each state a set of pairs. A pair for each action in \actions is included, declaring the amount of times the action is outgoing from that state. Note that $n$ is zero if an action is not outgoing from a state.
...
According to Definition \ref{def:eqrelview} the equivalence classes of \eqrelview on \states are 
\[
...
The \viewN \viewstrongoutactident, as well as \viewminoutaction, \viewmaxoutaction, \viewspanoutaction were initially motivated by transition systems in which there exist no probabilities but several actions outgoing from a single state may occur. As \mdpsN can model transition systems they were included. For \mdpsN with proper probability distributions, counting outgoing actions may help less. Instead one could disregard the amount of times an action is outgoing and only be interested in states that have the same options for the nondeterministic choice, before the probabilistic one.
...
\noindent According to Definition \ref{def:view} the set $\states' := \bigcup_{\state \in \states} \eqclassv$ is the set of states of \viewweakoutactident.
...
\begin{exmp}
	In Figure \ref{fig:outActSetSize} we can observe the \viewN $\viewoutactsetsize$ on \chgph. In the simplified representation of \achgphN on the states $\state_1, \state_2, \state_3$ have the outgoing actions $\action, \actionb, \actionc$, the state $\state_4$ has the outgoing action \action and the state $\state_5, \state_6$ have the outgoing action \actionc. Hence, the number of outgoing actions for $\state_1, \state_2, \state_3$ is three and for $\state_4, \state_5, \state_6$ is one. This is why each of these states become a new single state in the \viewN \viewoutactsetsize.
\end{exmp}
...
The sepcialcase of there only being one single ougoing action is worth a distinct view, since it hides all not nondeterministic choices, but nothing more.
...
Note that this \viewN is disregarding \notppty and hence groups states \emph{have} the property. This version was chosen because the intention of the \viewN is to hide \states in which there is no nondeterministic selection of an action. 
...
Analogously to Outgoing Actions \viewsN of utilizing incoming actions are feasable. Since there is no difference apart from the definitions itself, we only provide the definitions.
...
The DNF allows us to specify a requirement in disjunctive normal form about variables. States are mapped to the same value depending on whether they meet this requirement.
...
\noindent Analogously a \viewN based on a conjunctive normal formal can be defined that may be more convenient, depending on the requirement/restriction.
...
Apart from direct and indirect components of an \mdpN there are other values linked to states. These can be given by a reward structure or results from model checking. A reward structure simply assigns values to each state. Results from model checking are also available statewise. 
...
The smaller the granularity $r$ the more elements the partition has, and vice versa. In the implementation for each state \state the value $r \cdot i$ of $\propfct(\state)$ is directly determined using rounding. The formalization as in the definition above was chosen, because it clearly formalizes the rounding behavior. Two states $\state_1, \state_2$ are grouped if $\gfctprop(\state_1) = \gfctprop(\state_2)$. Hence, the obtained equivalence classes are
\begin{align*}
...
\noindent According to Definition \ref{def:view} the set $\states' := \bigcup_{\state \in \states} \eqclassv$ is the set of states of \viewprop.
...
Considering distances in a graph can be very helpful to get an overview of a graph. Likewise it helps a lot with understanding the structure of \achgphN. In order to consider the distance between states in a \achgphN we will need to formalize it. For this we will introduces \emph{paths} which conceptually are very similar to execution fragments \cite[Def. 2.4.]{Baier2008}.
...
For a given \chgphN \chgph with state set \states and $\state \in \states, \smstates \subseteq \states$ we write $\distpath(\mdp, \smstates, \state)$ short for $\distpath(\mdp, \smstates, \{\state\})$ and $\distpath(\mdp, \state, \smstates)$ short for $\distpath(\mdp, \{\state\}, \smstates)$. We do so in the same way for \distpathbi. The symbol $\infty$ is used to indicate there that there is no path and hence the distance is "infinite". We will compare against the symbol with equality to rule out whether there exists a path from one state state set to another.
To apply the notion of distance to the whole \mdpN graph with a view, it has to be in a way where some value related to a distance is computed for each state. Hence, some set of states is needed, from which on the distance is measure for the remaining states. Such set could for example be the set of initial states or any other set of interest. In each \viewN utilizing distance it is denoted with $\bar{\states}$. Moreover, we will take a granularity $\grandist \in \natnums$ to account. The following \viewN groups states that are reachable with the same amount of transitions $\bar{\states} \subseteq \states$ considering the granularity \grandist.
...
The number $n$ from the definition is the granularity of the distance cluster. If for example if the granularity is two ($n = 2$) the expression $d - (d \text { mod } n)$ for $d = 0$ evaluates to $0 - 0 = 0$, for $d = 1$ evaluates to $1 - 1 = 0$, for $d = 2$ evaluates to $2 - 0 = 2$ and for $d = 3$ evaluates to $3 - 1 = 2$ where each time the subtrahend is $d \text{ mod } 2$. We see that, \gfctdistance maps $\distpath(\chgph,\smstates, \state)$, if not infinity, to next smaller natural number that is dividable by $n$, which means the granularity causes that $\gfctdistance(\state)$ is a multiple of $n$.
...
A probable use case of a \viewN is to find out how a certain state is or was reached. For this we define a \viewN that groups states that have the same distance from a given state, when only traversing transitions backwards.
...
Note that the only difference of \viewdistancerev to \viewdistance is that it is $\distpath(\chgph,\state,\bar{\states})$ instead of $\distpath(\chgph,\bar{\states}, \state)$. According to Definition \ref{def:distance} this declares that paths from \state to $\bar{\states}$ are considered instead of a paths from $\bar{\states}$ to \state. Hence, the distance from set $\bar{\states}$ is measured with traversing transitions backwards. This causes states reaching $\bar{\states}$ with the same amount of transitions being grouped. Even though on a formal level, equality, equivalence classes and the new state set behave and are constructed analogously to the \viewN \viewdistance.
...
In the implementation, a simple breadth first search is used to determine the distance values from a given state set to each state. There is no algorithm such as Dijkstra needed as the weights on edges are probabilities and thereby no value interpretable as cost or distance. The implementation ensures that for every state $\state$ there exists a pair $(\state, \distval)$ in \fctdistdefault, i.e. $\infty$ if there exists no path to a given state. The algorithm in java code using functional programming can be found in the appendix.

\subsubsection{Cycles}
A cycle is a structure that can exist in every graph. The concept of cycles is not limited to \chgphsN or related models used in model checking. The thesis aims to discuss \viewsN that utilize domain specific knowledge or general concepts that are of special relevance when exploring \achgphN. In general cycles are worth exploring because they may lead to unwanted infinite results in model checking. Additionally, when seeking \viewsN, the main cycle idea can be adjusted to include \mdpsN characteristics. To formalize \viewsN based on cycles, we need to formalize the cycle concept itself. We'll define it in a way that suits our specific domain the best.
...
This \viewN groups all states that are contained in a cycle with a length of at least $n$. Hence, this \viewN can also finds self loops for $n \in \natnums$. It is to note that the affinity of a state to one or several respective cycles is lost. 
...
\end{align*}
\noindent According to Definition \ref{def:view} the set $\states' := \bigcup_{\state \in \states} \eqclassv$ is the set of states of \viewexactcycle.
...
\end{definition}
With this \viewN each state \state is mapped to the set of states. The set contains all the states from cycles, where the state \state is contained. According to Definition \ref{def:eqrelview} the equivalence classes of \eqrelview on \states are
\begin{align*}
...
\noindent A special kind of strongly connected components is the the bottom strongly connected component. Thus, they can be interesting for deadlock detection and liveness analysis.
...
In the implementation strongly connected components are determined using the algorithm of Gabow, afterwards filtering those \sccsN that have only transitions to states within the \sccN. Only the bottom strongly connected components remain.
