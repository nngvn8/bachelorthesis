...
%\begin{definition}
%	Let $\chgph = \chgphtuple$ be \achgphN, $\smstates \subseteq \states$, $\var \in \vars$ and $\varval \in \varevalimg$. The view \viewparamvaluesneq is defined by its \grpfctN where $\gfctsubparamvaluesneq : \smstates \to \imggrp$ with
%	\[
%	\state \mapsto
%	\begin{cases}
...
\begin{definition}
		Let $\chgph = \chgphtuple$ be \achgphN, $\smstates \subseteq \states$, $n \in \natnums$ and $\smstates \subseteq \states$ arbitrary. \redcomment{$\fctdistdefault$ IMPLEMENTATION IN PSEUDOCODE? DEFINITION BEFORE?} The \viewN \viewdistance is defined by its \grpfctN where $\gfctsubdistance : \smstates \to \imggrp$ with 
		\[
