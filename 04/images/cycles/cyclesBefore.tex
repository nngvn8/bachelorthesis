...
\begin{document}
	...
	\begin{tikzpicture} [every initial by arrow/.style={thick}]
		...
		(\basex+2,		\basey+4) 		node[mstate] (s2) 	{$\state_2$} 
		(\basex+5,		\basey+3.5) 	node[mstate, initial above,initial text=,initial distance=18pt] (s3) 	{$\state_3$} 
		...
		\path [trans] (s8) 	edge node [midway,below left]{\actionb} 			(s1);
