...
\begin{document}
	...
	\begin{tikzpicture} [scale=2, every initial by arrow/.style={thick}]
		...
		(\basex+1.5,	\basey-2) 		node[gstate] (s3) 	{$\state_3$} 
		(\basex,		\basey-2) 		node[gstate] (s4) 	{$\state_4$} 
		
		;
		
		%		
		\path [trans, bend left=30] 	(sbot) 	edge node [near end,above right=-2pt]	{\actionb} 		(s3);
		\path [trans, bend right=20] 	(sbot) 	edge node [midway,below left]	{\action} 		(s3);
		\path [trans] 		(sbot) 	edge node [midway,above left]	{\actionc} 		(s4);
		\path [trans] 		(s3)	edge node [midway,above right=-3pt]	{\actionc} 		(sbot);
