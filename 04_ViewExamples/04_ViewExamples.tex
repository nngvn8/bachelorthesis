...
\begin{definition}
	Let $\ts = \transitionsystem$ be a \chosengraphtypeN. The view \viewminoutaction is defined by its \grpfctN $\gfctminoutaction : \states \to \arbset$ with
	...
	where $\imggrp := \setoutact$, $\numoutact \in \natnums$ is the minimum amount of times an transition with action \action has to be outgoing in order to be grouped with the other states
	\[
...
\begin{definition}
	Let $\ts = \transitionsystem$ be a \chosengraphtypeN. The view \viewmaxoutaction is defined by its \grpfctN $\gfctmaxoutaction : \states \to \arbset$ with
...
\begin{definition}
	Let $\ts = \transitionsystem$ be a \chosengraphtypeN. The view 
...
\begin{align*}
	...
	\states_2 &:= \bigcup_{\state \in \states \setminus \states_1}\{\state\}.
\end{align*}
If states are to be grouped with the requirement of several parameters equaling specified values this can be achieved by using \parllcompN. \redcomment{disjunctive??}
