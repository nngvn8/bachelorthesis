...
\begin{definition}
	Let $\chgph = \chgphtuple$ be \chosengraphtypeN. The view \viewinitstates is defined by its grouping function \gfctinitstates \grpfctN with $\gfctinitstates : \states \to \imggrp$ with 
...
\begin{definition}
	Let $\chgph = \chgphtuple$ be \chosengraphtypeN. Let $\fctfindcycle(\chgph, n)$ be a function that returns a set $\cycleset$ of all cycles in \chgph where for all $\cycle \in \cycleset$ it is $|\{\state \in \states \mid \state \in \cycle\}| \geq n \in \natnums$. That is there are at least $n$ states contained in each cycle.
\end{definition}
...
\section{Comparison of the Examples}
\end{document} 
