...
\begin{definition}
	...
	Let \chgph = \chgphtuple be \chosengraphtypeN and $\grpfct$ a \grpfctN. A \emph{\viewN} is \chosengraphtypeN $\view{\viewppty} = \chgphtupledist$ that is derived from \mdp with the \grpfctN $\grpfct$ where
...
The identifier \viewppty is inherited from $\grpfct$ to \view{\viewppty}. It thereby uniquely determines \view{\viewppty} and declares that \view{\viewppty} is based on $\grpfct$. Based on denotes that $\grpfct$ is the \grpfctN of \view{\viewppty} referred to in Definition \ref{def:view}.
